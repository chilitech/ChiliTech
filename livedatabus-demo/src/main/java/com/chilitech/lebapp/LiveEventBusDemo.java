...
import com.chilitech.lebapp.activity.ObserverActiveLevelActivity;
import com.chilitech.lebapp.activity.PostDelayActivity;
import com.chilitech.lebapp.activity.StickyActivity;
import com.chilitech.lebapp.databinding.ActivityLiveDataBusDemoBinding;
import com.chilitech.lebapp.event.DemoEvent;
import com.chilitech.lebapp.service.IpcService;
...
public class LiveEventBusDemo extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LiveDataBus.get(DemoEvent.class)
    ...
    private void testMessageSetBeforeOnCreate() {
        //先发出一个消息
        LiveDataBus.get(KEY_TEST_MSG_SET_BEFORE_ON_CREATE, String.class).post("msg set before");
        //然后订阅这个消息
        LiveDataBus
    ...
    public void closeAll() {
        LiveDataBus.get(KEY_TEST_CLOSE_ALL_PAGE).post(true);
    }
    ...
    public void postValueCountTest() {
        ...
        for (int i = 0; i < sendCount; i++) {
            threadPool.execute(new Runnable() {
                @Override
                public void run() {
                    LiveDataBus.get(KEY_TEST_MULTI_THREAD_POST).post("test_data");
                }
            });
        }
    ...
    public void testMessageSetBefore() {
        ...
        //然后发出一个消息
        LiveDataBus.get(randomKey, String.class).post("msg set before");
        //然后订阅这个消息
        LiveDataBus
    ...
    public void sendMessageSetBefore() {
        LiveDataBus.get(randomKey, String.class).post("msg set after");
    }
